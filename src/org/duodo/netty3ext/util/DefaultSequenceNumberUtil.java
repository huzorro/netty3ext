...
public class DefaultSequenceNumberUtil {
	public static byte[] sequenceN2Bytes(SequenceNumber sn) {
		...
		long t = Long.parseLong(String.format("%1$02d%2$02d%3$02d%4$02d%5$02d",
				sn.getMonth(), sn.getDay(), sn.getHour(), sn.getMinutes(),
				sn.getSeconds()));
	...
	public static SequenceNumber bytes2SequenceN(byte[] bytes) {		
		long nodeIds = ByteBuffer.wrap(Arrays.copyOfRange(bytes, 0, 4))
				.getInt() & 0xFFFFFFFFL;
		String t = String.format("%1$010d", ByteBuffer.wrap(
				Arrays.copyOfRange(bytes, 4, 8)).getInt() & 0xFFFFFFFFL);
		long sequenceId = ByteBuffer.wrap(Arrays.copyOfRange(bytes, 8, 12))
				.getInt() & 0xFFFFFFFFL;
